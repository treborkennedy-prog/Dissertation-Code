...
def grid_from_name(name):
    ...
    if name == "pKa_stage2":
        return {
            ...
            "rf__max_features": [0.7, 0.6, 0.5, 0.4, 0.3],
        }
    if name == "logS_stage2":
        return {
            "feature_set": ["combined"],
...
def evaluate_config(X_dev, y_dev, groups, params, n_splits, cv_mode, n_jobs, random_state):
    ...
    for tr, va in splits:
        ...
        model = RandomForestRegressor(
            ...
            max_depth=None if params["rf__max_depth"] is None else int(params["rf__max_depth"]),
            min_samples_split=int(params["rf__min_samples_split"]),
            min_samples_leaf=int(params["rf__min_samples_leaf"]),
...
def main():
    ...
    best_params = {
        ...
        "rf__n_estimators": int(best["rf__n_estimators"]),
        "rf__max_depth": None if pd.isna(best["rf__max_depth"]) else int(best["rf__max_depth"]),
        "rf__min_samples_split": int(best["rf__min_samples_split"]),
        "rf__min_samples_leaf": int(best["rf__min_samples_leaf"]),
        "rf__max_features": best["rf__max_features"],
    }
    ...
    model = RandomForestRegressor(
        n_estimators=int(best_params["rf__n_estimators"]),
        max_depth=best_params["rf__max_depth"],
        min_samples_split=int(best_params["rf__min_samples_split"]),
        min_samples_leaf=int(best_params["rf__min_samples_leaf"]),
